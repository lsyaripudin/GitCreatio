namespace Terrasoft.Configuration.SsoSettings
{
	...
	internal class BaseSsoVirtualSettingsQueryExecutor : IEntitySchemaQueryExecutor
	{
		...
		private Entity CreateVirtualEntity(Entity settings, EntitySchemaQueryColumn nameColumn,
				EntitySchemaQueryColumn codeColumn, EntitySchemaQueryColumn templateIdColumn,
		...
		public EntityCollection GetEntityCollection(EntitySchemaQueryFilterCollection filters) {
			...
			foreach (Entity settings in settingsCollection) {
				Entity entity = CreateVirtualEntity(settings,
					nameColumn, codeColumn, templateIdColumn, templateNameColumn);
